import { Star } from "lucide-react";

type Props = {
  stars: number;
  reviews: number;
  variant?: "light" | "solid";
  className?: string;
};

export function GoogleBadge({
  stars,
  reviews,
  variant = "light",
  className = "",
}: Props) {
  const isSolid = variant === "solid";

  return (
    <div
      className={`inline-flex items-center gap-3 rounded-full border ${
        isSolid
          ? "border-transparent bg-white shadow-card"
          : "border-brand-500/10 bg-white/80 backdrop-blur"
      } px-4 py-2 ${className}`}
      aria-label={`Calificación ${stars} de 5 estrellas con ${reviews} reseñas en Google`}
    >
      <span className="grid h-7 w-7 place-items-center rounded-full bg-white shadow-sm ring-1 ring-black/5">
        {/* Google "G" */}
        <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.25 1.06-3.71 1.06-2.86 0-5.29-1.93-6.15-4.53H2.18v2.84A11 11 0 0 0 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.85 14.1A6.6 6.6 0 0 1 5.5 12c0-.73.13-1.43.35-2.1V7.07H2.18A11 11 0 0 0 1 12c0 1.78.43 3.46 1.18 4.93l3.67-2.83z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.07.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.67 2.83C6.71 7.31 9.14 5.38 12 5.38z"
          />
        </svg>
      </span>
      <div className="flex items-center gap-2">
        <span className="flex" aria-hidden="true">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star
              key={i}
              className={`h-3.5 w-3.5 ${
                i < Math.round(stars)
                  ? "fill-accent-500 text-accent-500"
                  : "fill-ink-muted/30 text-ink-muted/30"
              }`}
            />
          ))}
        </span>
        <span className="text-sm font-semibold text-ink">
          {stars.toFixed(1)}
        </span>
        <span className="text-xs text-ink-muted">·</span>
        <span className="text-xs font-medium text-ink-soft">
          {reviews} reseñas
        </span>
      </div>
    </div>
  );
}
